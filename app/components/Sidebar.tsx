      ...
      <AnimatePresence>
        {isMobileOpen && (
          <>
            ...
            >
              {/* Mobile-specific content with labels always visible */}
              <>
                {/* Logo Section */}
                <div className="flex items-center justify-between p-4 border-b">
                ...
                {/* Navigation Items - Always show labels on mobile */}
                <nav className="flex-1 p-2 space-y-1 overflow-y-auto">
                  {navigationItems.map((item) => {
                    const Icon = item.icon
                    const active = isActive(item.href)
                ...
                <div className="p-4 border-t">
                  ...
                  <button
                    onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
                    className="w-full flex items-center gap-3 px-3 py-2 rounded-lg mb-2 hover:bg-accent hover:text-accent-foreground transition-all"
                  >
                    {theme === 'dark' ? (
                      <Sun className="h-5 w-5 flex-shrink-0" />
                    ) : (
                  ...
                  <button
                    onClick={handleLogout}
                    className="w-full flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-destructive/10 hover:text-destructive transition-all"
                  >
                    <LogOut className="h-5 w-5 flex-shrink-0" />
                    <span className="text-sm">Logout</span>
                  </button>
